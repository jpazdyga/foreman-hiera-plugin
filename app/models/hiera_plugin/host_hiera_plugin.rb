require "open3"
module HieraPlugin
  module HostHieraPlugin
    extend ActiveSupport::Concern   
    # include Open3

    HIERA_VARIABLE_REGEX = /^[A-Za-z0-9.:_\-@]+$/
    def get_hiera_plugin_variable_value(hiera_var)
      hiera_plugin_outpu = {"error" => {"message" => "Invalid input"}}
      if HIERA_VARIABLE_REGEX.match(hiera_var.to_s)
        ...
        begin
          ...
          if stderr_output.split("\n").any?{|res| !res.include?("DEBUG:")}
            hiera_plugin_outpu = {"error" => {"output" => stderr_output}}
          else
            secured_backends = if SETTINGS[:hiera_plugin] and SETTINGS[:hiera_plugin][:secured_backend]
              SETTINGS[:hiera_plugin][:secured_backend].is_a?(Array) ? SETTINGS[:hiera_plugin][:secured_backend] : [SETTINGS[:hiera_plugin][:secured_backend]]
            else
              []
            end
            hiera_plugin_outpu = {}
            hiera_plugin_outpu.merge!(parse_hiera_plugin_output(stderr_output, hiera_var))
            if hiera_plugin_outpu["found"] and first_found = hiera_plugin_outpu["found"].first and secured_backends.include?(first_found["backend"])
              hiera_plugin_outpu.merge!({"value" => "[MASKED]"})
            else
              hiera_plugin_outpu.merge!({"value" => value})
            end
          end
        rescue Exception => e
          hiera_plugin_outpu = {"error" => {"message" => e.message}}
        end        
      end
      return hiera_plugin_outpu
    end


    def parse_hiera_plugin_output(hiera_output, requested_key=nil)
      found = []
      # dependency = []
      backend = nil
      dependencies = []
    ...
    def get_hiera_backend_url(backend, level)
      path = level      
      backend_url_template = if !backend.to_s.empty? and SETTINGS[:hiera_plugin] and SETTINGS[:hiera_plugin][:url_templates].is_a?(Hash)
        SETTINGS[:hiera_plugin][:url_templates][backend.to_s.to_sym]
      else
        ""
      end
